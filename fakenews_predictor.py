...
def Data_Display():
    ...
    data1 = pd.read_csv(r"F:\fake_news_detection\train.csv", encoding='unicode_escape')

    data1.shape
    #return a table/tuple of the data frame
...
def Train():
    ...
    result = pd.read_csv(r"F:\fake_news_detection\train.csv",encoding = 'unicode_escape')

    result.head()
        
    # result['headline_without_stopwords'] = result['statment'].apply(lambda x: ' '.join([word for word in x.split() if word not in (stop)]))
    ...
    # result.head()
    # result['pos']
    x_train, x_test, y_train, y_test = train_test_split(result['statment'], result['Result'],
