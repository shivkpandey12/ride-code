...
class main:
    def __init__(self, master):
        #self is a pointer using which master page is created and root is created
        # Window
        self.master = master
        ...
        # Create Widgets
        self.widgets()
        #widget to create dialogue boxes and functionable buttons
    ...
    def login(self):
        ...
        if result:
            ...
            call(['python','fakenews_predictor.py'])
 ...
 # , relwidth=1, relheight=1)
    ...
    # Draw Widgets
    def widgets(self):
        self.head = tk.Label(self.master, text='Welcome To Login', background="#EDDA74", font=('Times New Roman', 20),
                             pady=20)
        self.head.pack()
        ...
        # self.head.pack()
        self.logf = tk.Frame(self.master, padx=50, pady=50, background="#EDDA74")
        tk.Label(self.logf, text='Username: ', background="#EDDA74", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.username, bd=5, background="white", font=('', 15)).grid(row=0, column=1)
        tk.Label(self.logf, text='Password: ', background="#EDDA74", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.password, bd=5, background="white", font=('', 15), show='*').grid(row=1,
                                                                                                                column=1)
        tk.Button(self.logf, text=' Login ', command=self.login, bd=2, font=("Times New Roman", 20), background="blue",
...
root.configure(background="#EDDA74")
